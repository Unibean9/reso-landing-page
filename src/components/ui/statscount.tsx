...
function AnimatedCounter({
  ...
  return (
    ...
    >
      ...
      <motion.p
        className={cn(
          'text-muted-foreground text-xs sm:text-sm leading-relaxed px-1 sm:px-2 hyphens-auto wrap-break-word',
        )}
          ...
          <span className='hidden sm:inline'>
            {title.includes('WITH') ? (
              <>
                ...
                <span
                  className={cn(
                    'text-uni-teal-800 dark:text-uni-teal-500 font-semibold',
                  )}
                >
                  {title.split('WITH')[1]}
                </span>
          ...
          {stats.map((stat, index) => (
            ...
            >
              ...
              {index < stats.length - 1 && showDividers && (
                <motion.div
                  className={cn(
                    'absolute -right-1 sm:-right-2 lg:-right-4 top-1/2 transform -translate-y-1/2 h-12 sm:h-16 lg:h-20 w-px bg-border',
                  )}

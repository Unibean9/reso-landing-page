...
interface LayerStackProps {
  ...
  /** Desktop: fit this many cards in the viewport (plus a small peek of the next). */
  visibleCards?: number;
  /** Pixels of the next card visible when using `visibleCards`. */
  peekAmount?: number;
...
function LayerStack({
  ...
  const effectiveCardWidth = React.useMemo(() => {
    ...
    if (visibleCards && visibleCards > 0 && containerWidth > 0) {
      const gaps = (visibleCards - 1) * cardGap;
      return Math.floor((containerWidth - gaps - peekAmount) / visibleCards);
    }
    return cardWidth;
  }, [isMobile, containerWidth, cardWidth, visibleCards, cardGap, peekAmount]);
    ...
    <div ref={ref} className={cn('w-full select-none', className)}>
      ...
      <div className='mt-4 flex flex-col items-center gap-2'>
        ...
        <p className="text-[10px] font-medium tracking-[0.14em] uppercase text-muted-foreground">
          {isMobile ? "Kéo ngang" : "Kéo hoặc cuộn ngang"}
        </p>
      </div>
    </div>
  );

...
export interface IphoneProps extends HTMLAttributes<HTMLDivElement> {
  imgUrl?: string;
  clip?: string;
  /** Fill edge-to-edge under the frame (no notch padding for children). */
  screenFullBleed?: boolean;
...
export function Iphone({
  ...
  return (
    <div
      className={cn('relative inline-block align-middle leading-none', className)}
    ...
    >
      ...
      {!clip && imgUrl && (
        <div
          className={`pointer-events-none absolute z-0 overflow-hidden bg-card ${screenClassName ?? ''}`}
          style={{
            left: `${calcX}%`,
            top: `${calcY}%`,
            width: `${calcW}%`,
      ...
      {children && (
        <div
          className={`absolute z-10 overflow-hidden bg-card ${screenClassName ?? ''}`}
          style={{
            left: `${calcX}%`,
            top: `${calcY}%`,
            width: `${calcW}%`,
        ...
        >
          <div
            className='size-full'
            style={screenFullBleed ? undefined : { paddingTop: `${notchHeight}%` }}
          >
            {children}
          </div>
        </div>

    ...
    >
      {showIcons && (
        <>
          <Icon
            className={cn(
              'absolute -left-3 -top-3 h-6 w-6 text-foreground',
              iconClassName,
            )}
          />
          <Icon
            className={cn(
              'absolute -bottom-3 -left-3 h-6 w-6 text-foreground',
              iconClassName,
            )}
          />
          <Icon
            className={cn(
              'absolute -right-3 -top-3 h-6 w-6 text-foreground',
              iconClassName,
            )}
          />
          <Icon
            className={cn(
              'absolute -bottom-3 -right-3 h-6 w-6 text-foreground',
              iconClassName,
            )}
          />
        </>
      ...
      {glowEffect && (
        <motion.div
          className='absolute inset-0 bg-linear-to-r from-uni-teal-800/10 via-uni-teal-500/10 to-uni-teal-300/10 opacity-0 blur-xl transition-opacity duration-500 group-hover/canvas-card:opacity-100'
          initial={{ opacity: 0 }}

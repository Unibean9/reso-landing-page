  ...
  return (
    <section id="features" className={cn("border-t border-border py-28 px-5", className)}>
      <div className="mx-auto max-w-6xl">
            ...
            {features.map((feat, i) => {
              ...
              return (
                <div
                  ...
                  className="min-w-0 shrink-0 basis-[88%] snap-center"
                >
                  <FadeIn delay={i * 0.05}>
                    <div
            ...
            <div className="mt-5 flex items-center justify-between gap-4">
              <div className="flex items-center gap-2">
                {features.map((feat, index) => (
                  <button
                    key={feat.title}
                    type="button"
                    aria-label={`Đi tới thẻ tính năng ${index + 1}`}
                    onClick={() => scrollToMobileCard(index)}
                    className={cn(
                      "h-2 rounded-full bg-foreground/15 transition-all",
                      mobileIndex === index ? "w-6 bg-foreground" : "w-2",
                    )}
                  />
                ))}
              </div>
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="flex size-10 items-center justify-center rounded-full border border-border bg-card text-foreground transition hover:border-foreground/20"
                >
        ...
        <div className="hidden grid-cols-1 gap-3 lg:grid lg:grid-cols-3 lg:auto-rows-fr">
          {features.map((feat, i) => {
            const Viz = feat.viz;
            const Icon = feat.icon;
            return (
